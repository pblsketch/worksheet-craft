...
## Linked function and data graphs, checked on 2026-09-21

Function groups share the LaTeX source with a restricted numeric parser and SVG plot. The dialog previews equations, ranges and a/b/c/k coefficients before atomic apply. Data-chart groups redraw bar or line graphs when their source table changes. Invalid input preserves the prior document; invalid table data blocks editor save/print and hides stale charts during browser printing.

Validation: 13 existing editor/math integration tests passed, followed by 4 graph tests covering numeric evaluation, discontinuities, new insertion, unlink/undo, chart type changes, live preview, coefficients, invalid-input handling, copy, save and offline reopen. Both shipped linked-graph demos passed offline browser tests (2 tests). Three representative existing lesson examples across the repositories passed edit/save/reopen and PDF checks; 31 other layout variants were not rerun. The graph dialog, edited data chart and one-page demonstration PDF were visually inspected. Ruff checks and formatting passed; JavaScript syntax checks found zero errors in 34 files. Both skill manifests and extracted Claude ZIP helpers passed validation.

The function renderer is a sampled approximation for supported y=f(x) expressions, not symbolic algebra. Existing arbitrary SVG/image graphs are not automatically linked. The linked-graphs demo is a feature demonstration, not a student worksheet. Actual generation inside the Claude service remains untested.
