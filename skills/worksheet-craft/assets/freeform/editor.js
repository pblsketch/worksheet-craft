...
(() => {
  ...
  function answerLines(){const el=ctx.answer,lines=+field('answer-lines').value,pitch=+field('answer-pitch').value;if(!valid(el))return;el.dataset.answerLines=lines;el.dataset.answerPitch=pitch;el.style.backgroundImage=lines?`linear-gradient(to bottom,transparent calc(${pitch}mm - 1px),#a9bdb2 0)`:'none';el.style.backgroundSize=`100% ${pitch}mm`;el.style.backgroundRepeat='repeat-y';if(lines)Object.assign(el.style,{minHeight:`${lines*pitch}mm`,height:'auto',boxSizing:'border-box',lineHeight:`${pitch}mm`,overflow:'visible'});}
